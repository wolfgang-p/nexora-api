...
-- account or a real device row. Relax both uploader columns so NULL is
-- allowed — ownership for these rows is implicit (the meeting host who
-- uploaded it). All existing rows stay populated; this only opens up
-- new inserts.
ALTER TABLE media_objects
  ALTER COLUMN uploader_user_id DROP NOT NULL;
ALTER TABLE media_objects
  ALTER COLUMN uploader_device_id DROP NOT NULL;
